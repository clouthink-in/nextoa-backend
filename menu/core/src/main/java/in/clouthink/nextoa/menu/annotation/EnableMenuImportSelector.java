...
import in.clouthink.nextoa.menu.core.*;
import in.clouthink.nextoa.rbac.model.Action;
import in.clouthink.nextoa.rbac.model.DefaultAction;
import in.clouthink.nextoa.menu.core.Menu;
import in.clouthink.nextoa.rbac.model.*;
...
public class EnableMenuImportSelector implements ImportSelector {
	...
	public static class MenuPluginBeanRegistrar implements ImportBeanDefinitionRegistrar {
		...
		private List<in.clouthink.nextoa.menu.core.Menu> toMenu(AnnotationAttributes[] menu) {
			return Stream.of(menu).map(item -> toMenu(item)).collect(Collectors.toList());
		}

		private Menu toMenu(AnnotationAttributes menu) {
		...
		private List<in.clouthink.nextoa.rbac.model.Action> toAction(AnnotationAttributes[] actions) {
			return Stream.of(actions).map(item -> toAction(item)).collect(Collectors.toList());
		}

		private in.clouthink.nextoa.rbac.model.Action toAction(AnnotationAttributes action) {
