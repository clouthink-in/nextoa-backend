...
import in.clouthink.synergy.menu.core.*;
import in.clouthink.synergy.rbac.model.DefaultAction;
import in.clouthink.synergy.menu.core.Menu;
...
public class EnableMenuImportSelector implements ImportSelector {
	...
	public static class MenuPluginBeanRegistrar implements ImportBeanDefinitionRegistrar {
		...
		private List<in.clouthink.synergy.menu.core.Menu> toMenu(AnnotationAttributes[] menu) {
		...
		private List<in.clouthink.synergy.rbac.model.Action> toAction(AnnotationAttributes[] actions) {
			return Stream.of(actions).map(item -> toAction(item)).collect(Collectors.toList());
		}

		private in.clouthink.synergy.rbac.model.Action toAction(AnnotationAttributes action) {
