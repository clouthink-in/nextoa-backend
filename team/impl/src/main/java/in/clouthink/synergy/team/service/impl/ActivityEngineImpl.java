...
public class ActivityEngineImpl implements ActivityEngine {
    ...
    public void markActivityAsRead(Activity activity, User user) {
        ...
        if (task != null) {
            messageRepository.markAsRead(task.getId());
        }
    ...
    public void handleStartActivityAction(ActivityAction startActivityAction) {
        ...
        Task task = messageRepository.findByBizRefIdAndReceiver(activity.getId(), activity.getCreatedBy());
        if (task == null) {
            Date now = new Date();
            task = new Task();
    ...
    public void handleRevokeActivityAction(ActivityAction revokeActivityAction) {
        ...
        List<Task> taskList = messageRepository.findByBizRefId(reloadActivity.getId());
    ...
    public void handleEndActivityAction(ActivityAction endActivityAction) {
        ...
        Task task = messageRepository.findByBizRefIdAndReceiver(endActivityAction.getActivity().getId(),
    ...
    public void handleTerminateActivityAction(ActivityAction activityAction) {
        ...
        List<Task> taskList = messageRepository.findByBizRefId(activityAction.getActivity().getId());
    ...
    private void handleMessage(ActivityAction activityAction) {
        //
        Task task = messageRepository.findByBizRefIdAndReceiver(activityAction.getActivity().getId(),
...
//	}
    ...
    private Task createOrUpdateMessage(Activity activity, ActivityAction activityAction, User sender, User toUser) {
        Task task = messageRepository.findByBizRefIdAndReceiver(activity.getId(), toUser);
        Date now = new Date();
        if (task == null) {
            task = new Task();
            task.setCategory(activity.getCategory());
